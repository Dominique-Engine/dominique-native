...
#if !defined(DENGINEDX11_HELPERS)
#define DENGINEDX11_HELPERS
...
namespace DEngine
{
    ...
    void CreateRenderTarget( DEDirectX11Context &dxContext)
    {
        ...
        dxContext.pd3dDevice->CreateRenderTargetView(pBackBuffer, NULL, &dxContext.mainRenderTargetView);
        pBackBuffer->Release();
    }

    bool CreateDeviceD3D(HWND hWnd, DEDirectX11Context &dxContext, const bool debug = false)
    {
        // Setup swap chain
        ...
        if (debug) createDeviceFlags |= D3D11_CREATE_DEVICE_DEBUG;//TODO not working in debug mode
        ...
        if (D3D11CreateDeviceAndSwapChain(NULL, 
                                        ...
                                        D3D11_SDK_VERSION, 
                                        &sd, 
                                        &dxContext.pSwapChain, 
                                        &dxContext.pd3dDevice, 
                                        &featureLevel, 
                                        &dxContext.pd3dDeviceContext
            ) != S_OK)
        {
            return false;
        }
